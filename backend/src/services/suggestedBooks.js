...
async function getMultiple(page = 1) {
    try {
        ...
        const rows = await (0, db_1.default)("CALL sp_get_suggested_books(?, ?)", [
            offset,
...
async function suggest(book) {
    ...
    try {
        ...
        const result = await (0, db_1.default)("CALL sp_insert_suggested_book(?, ?, ?, ?, ?, ?)", [
            book.isbn,
            book.title,
            book.author,
            book.description,
        ...
        if (affectedRows) {
            ...
            throw new helper_1.AppError("Creation of suggested book failed", 501, {
