    ...
    return (
        <div className="bg-white shadow-md rounded-2xl p-4 border border-gray-200 transform transition duration-300 hover:scale-105">
            {"title" in data && typeof data.title === "string" && (
                <h2 className="text-lg font-semibold mt-3">{data.title}</h2>
            )}
            ...
            {"ban_reason" in data && data.ban_reason !== null && (
                <p className="text-red-500 text-xs mt-2">Ban Reason: {String(data.ban_reason)}</p>
            )}
            {"banned_by" in data && data.banned_by !== null && (
                <p className="text-red-500 text-xs">Banned By: {String(data.banned_by)}</p>
            )}
